...
st.set_page_config(page_title="Inventario | SD", page_icon="📦", layout="wide")
inject_css()
...
# ─ Evolución de quiebres por semana ────────────────────────────────────
st.markdown("#### 📉 Historial de Quiebres de Stock por Semana")
st.caption("Los picos coinciden con Buen Fin, Navidad y Día de las Madres — exactamente cuando más duelen")
...
with right:
    ...
    for _, row in perdidas_fmt.iterrows():
        temp_color = {"Buen Fin": "#F59E0B", "Navidad": "#EF4444", "Día de las Madres": "#EC4899", "Hot Sale": "#8B5CF6", "Regular": "#6B7280"}.get(row["temporada"], "#6B7280")
        st.markdown(f"""
            ...
            <div style="display:flex;justify-content:space-between;align-items:flex-start">
                <div>
                    ...
                    <div style="font-size:0.78rem;color:#9CA3AF;margin-top:0.1rem">
                        {row['quiebre_inicio']} → {row['quiebre_fin']} · 
                        <span style="color:{temp_color};font-weight:600">{row['temporada']}</span> · 
                        {row['dias_sin_stock']} días sin stock
                    </div>
                </div>
...
st.caption("Los SKUs Clase A generan el 80% del margen. El resto consume almacén sin retorno proporcional.")
...
with col2:
    st.warning(f"⚠️ **{len(skus_c)} SKUs Clase C** generan solo ~5% del margen pero consumen espacio de almacén. Evalúa reducir inventario o renegociar condiciones.")
